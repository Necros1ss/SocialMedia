...
const FeedItem = ({ userId, post, openPost, goBack, isNested = false }) => {
  ...
  const handleShare = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to share: ${response.status} - ${errorText}`);
      }
    } catch (error) {
      console.error('Error sharing post:', error);
      alert('Error sharing post. Check console for details.');
    }
  };
  return (
    <div key={post.id} className="collection-card post-card">
      ...
      <div className="post-footer">
        <Reaction
          ...
          targetType="POST"
        ></Reaction>
        <button className="highlight-item" onClick={() => openPost(post)}>
          <svg
            rpl=""
            aria-hidden="true"
        ...
        {!isNested && (
          <button className="highlight-item" onClick={onShare}>
            <svg
              rpl=""
              aria-hidden="true"
            ...
            <div className="highlight-title">{post.shareCount || 0} Shares</div>
          </button>
        )}
      </div>
      ...
      {shareModalOpen && (
        <div className="modal-overlay" onClick={() => setShareModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h2>Share Post</h2>
            <p style={{marginBottom: '10px', color: '#666'}}>Original: {title}</p>
            <textarea
              placeholder="Add a caption for your share..."
              value={shareCaption}
